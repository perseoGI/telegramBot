...
def todolist_item_keyboard(limit=None, completed=None):
    ...
    keyboard[0] = [
        InlineKeyboardButton(
            _("Un-complete"), callback_data=str(Callback.ACTION_UNCOMPLETE)
        )
        if completed
...
def todocategory_options_keyboard():
    ...
    keyboard.append(
        InlineKeyboardButton(
            _("Create category"), callback_data=Callback.CREATE_CATEGORY
        )
    )
    keyboard.append(
        InlineKeyboardButton(
            _("Delete category"), callback_data=Callback.DELETE_CATEGORY
        )
    )
    keyboard.append(
        InlineKeyboardButton(_("Edit category"), callback_data=Callback.EDIT_CATEGORY)
    )
