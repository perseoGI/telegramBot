from database import mark_todo_as_completed, set_todo_deadline, get_pending_background_todo_id_and_description, set_pending_background_todo_id
...
def background_todo_postpone(update, context):
    ...
    (todo_id, todo_description) = get_pending_background_todo_id_and_description(chat_id, user_id, message_id)
...
background_response_handler = ConversationHandler(
    entry_points=[CallbackQueryHandler(background_response_callback, pattern='^todo_deadline_achieved-(\w+)-(\d+)$')],
    states={
