...
class Todo(BaseTable):
    ...
    id = IntegerField(primary_key=True)
    description = TextField(not_null=True)
    id_category = IntegerField(foreign_key=Category.id)
    id_user =
        #IntegerField(foreign_key=User.id)
...
if __name__ == '__main__':
    ...
    with Database("test.db") as db:
        ...
        #insert data
        db.query().insert(user1, user2, user3, cat1).execute()
